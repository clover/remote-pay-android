...
public class PreAuthDialogFragment extends DialogFragment {
  ...
  public void onContinue() {
    ...
    if(preauthName.getText().toString().length()>0 && CurrencyUtils.convertToLong(preauthAmount.getText().toString()) > 0L){
    ...
    if(errorMessage.size() > 0){
      ...
      ((ExamplePOSActivity)getActivity()).showPopupMessage(null, errorMessage.toArray(errorArray), false);
    }
  }
